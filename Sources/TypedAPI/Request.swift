...
/// An HTTP network request with typed success and failure.
public struct Request<Success, Failure: RequestError>: @unchecked Sendable {
    /// HTTP method, e.g. "GET".
    ...
    public func withResponse<T>(_ type: T.Type) -> Request<T, Failure> {
        var copy = Request<T, Failure>(optionalUrl: url, method: method)
        copy.query = query
        copy.body = body
...
extension Request where Success == Void {
    /// Initializes the request with the given parameters.
    public init(
        url: URL,
        method: HTTPMethod = .get,
